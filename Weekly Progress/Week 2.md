# Weekly Progress Summary

Week: 1 of 12 | Dates: 02/12/2026 to 02/15/2026

## Weekly Metrics

| Metric                 | Target | Actual | Status |
| ---------------------- | ------ | ------ | ------ |
| Training Hours         | 15-20  | 14     | ✗      |
| Challenges Solved      | 15-25  | 11     | ✗      |
| Live CTF Participated  | 1-2    | 0      | ✗      |
| CTF Challenges Solved  | 2-5    | 0      | ✗      |
| Writeups Created       | 3-5    | 11     | ✓      |
| New Techniques Learned | 2-3    | 4      | ✓      |

## Weekly Points Breakdown

| Source               | Points |
| -------------------- | ------ |
| Training Hours       | 65     |
| Challenges Completed | 240    |
| Quality Bonuses      | 180    |
| Live CTF Performance | 0      |
| Writeups & Learning  | 0      |
| **Weekly Total**     | 485    |

## Category Distribution (Hours This Week)

```
Web Exploitation:     _____ hours [========] ____%
Binary Exploitation:  _____ hours [========] ____%
Cryptography:        _____ hours [========] ____%
Reverse Engineering: _____ hours [========] ____%
Forensics:           _____ hours [========] ____%
OSINT:               14 hours [========] 100%
```
## Skill Level Assessment (Self-Rated 1-10)

| Specialty  | Last Week | This Week | Change |
| ---------- | --------- | --------- | ------ |
| Primary #1 (OSINT) | 3 | 5 | +2 |
| Primary #2 (Web)   | 2 | 3 | +1 |
| Secondary          | 2 | 3 | +1 |

## Weekly Achievements

[x] Solved first challenge in new category  
[x] Completed difficult challenge independently  
[x] Helped teammate solve challenge  
[x] Found new technique/exploit  
[ ] Improved solve time by 20%+  
[ ] Other: _____________________  

## Weekly Challenges & Lessons

Biggest challenge this week:
Filtering relevant OSINT data and validating multiple sources without wasting time.

How you overcame it (or plan to):
Used structured note-taking, cross-verification (reverse image + map validation), and slowed down analysis before confirming answers.

Most valuable lesson learned:
OSINT success depends more on observation and correlation than technical exploitation.

Adjustment for next week:
Increase total training hours to at least 15, introduce Web Exploitation challenges, and attempt at least one Hard-level room.
